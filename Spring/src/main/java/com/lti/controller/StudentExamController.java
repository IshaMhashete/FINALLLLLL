...
@Controller
@SessionAttributes({"presenStudentReportID","resume","dummy","reportsubjectchoice","subjectchoice","questionslevel1","qnol1","counterlevel1","questionslevel2","qnol2","counterlevel2","questionslevel3","qnol3","counterlevel3","loggedInUser"})
public class StudentExamController {
	...
	public String subjectSessionVariable(ModelMap model,@RequestParam("link") String name) {
	 ...
	 @RequestMapping(path="/resumeExam.lti", method = RequestMethod.POST)
	  public String resumeExam(ModelMap model) {
		  ...
		  if(report.isEmpty())
		  {
			  return "noGivenTest.jsp";
		  }
		  else
		  {			  
			 int len=report.size()-1;
			 StudentSideReport studentReport=report.get(len);
			 model.put("presenStudentReportID", studentReport.getReportid());
			 model.put("counterlevel1",studentReport.getLevel1score());
			 model.put("counterlevel2",studentReport.getLevel2score());
			 model.put("counterlevel3",studentReport.getLevel3score());
			 if(studentReport.getLevel1score()==0 ||studentReport.getLevel1score()<=1) {
				 return "redirect:examlevel1.lti";
			 }
			 if(studentReport.getLevel2score()==0 ||studentReport.getLevel2score()<=1) {
				 return "redirect:examlevel2.lti";
			 }
			 if(studentReport.getLevel3score()==0 ||studentReport.getLevel3score()<=1) {
				 return "redirect:examlevel3.lti";
			 }
			 return "noPreviousLeftTest.jsp";
			  				
		  }
	 }
	 
	  @RequestMapping(path="/resumeReport.lti", method = RequestMethod.POST)
	  public String resumeReport(ModelMap model) {
		  int subid= Integer.parseInt((model.get("subjectchoice").toString()));
		  ...
		  Integer reportid=(Integer)model.get("presenStudentReportID");
		  studentexamService.AddResumestudentreport(reportid,count1,count2,count3);
...
//************************************************************************************************************************	
	@RequestMapping(path="/examlevel1.lti")
...
//********************************************************************************************************************		
	  ...
	  @RequestMapping(path="/examlevel2.lti")
...
//******************************************************************************************************************************		  
		  ...
		  @RequestMapping(path="/examlevel3.lti")
...
//************************************************************************************************************************
			  ...
			  public String studentadminreport(ModelMap model,@RequestParam("studentId") int id) {
				  ...
				  model.put("dummy", 1);
				  return "redirect:listviewSubjectstudentside.lti";				  
			  }
